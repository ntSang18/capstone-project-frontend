    ...
    <div id="transaction-overview" class="data-overview">
      ...
      <div class="data-table data-section">
        <el-table
          ...
          max-height="300"
        >
          <el-table-column label="Ngày nạp" prop="created_at" sortable width="180">
            <template #default="scope">
              <span>{{ dateTimeFormatter(scope.row.created_at) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="User id" prop="user.id" width="90" />
          <el-table-column label="Phương thức" prop="method" width="140" />
          <el-table-column label="Giá" prop="money" sortable width="140">
            <template #default="scope">
              <span>{{ toVnd(scope.row.money) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="Khuyến mãi" prop="discount" sortable width="140">
            <template #default="scope">
              <span>{{ scope.row.discount * 100 }}%</span>
            </template>
          </el-table-column>
          <el-table-column label="Thực nhận" prop="actual_money" sortable width="140">
            <template #default="scope">
    ...
    <div id="payment-overview" class="data-overview">
      ...
      <div class="data-table data-section">
        ...
        >
          ...
          <el-table-column label="Mã tin" prop="post.id" sortable width="90" />
          <el-table-column label="Số lượng" prop="number_package" width="140" />
          <el-table-column label="Tổng phí" prop="price" sortable width="140">
            <template #default="scope">
  ...
  data() {
    return {
      transaction: {
        list: [],
        data: {
          labels: ['MoMo', 'ZaloPay', 'VNPay'],
          datasets: [
            {
              backgroundColor: ['#B0006E', '#0090EC', '#E11921'],
              data: [],
            },
          ],
        },
      ...
      payment: {
        ...
        data: {
          ...
          datasets: [
            {
              backgroundColor: ['#E13427', '#EA2E9D', '#FF6600', '#3763e0', '#055699'],
              data: [],
            },
          ],
        },
  ...
  methods: {
    ...
    getTransactionDoughnutData() {
      METHODS.forEach(method => {
        const total = this.transaction.list.reduce((accumulator, currentValue) => {
    ...
    getPaymentDoughnutData() {
      TYPES.forEach(type => {
        const total = this.payment.list.reduce((accumulator, currentValue) => {
          if (currentValue.package_price.type === type.value) {
            let amount = currentValue.number_package * currentValue.package_price.price;
    ...
    getTransactionMonthlyData() {
      ...
      for (var i = 0; i <= 11; i++) {
        const total = this.transaction.list.reduce((accumulator, currentValue) => {
          ...
          if (time.getMonth() === i && time.getFullYear() === yearNow) {
            return accumulator + currentValue.money;
          }
          return accumulator;
        }, 0);
    ...
    getMonthTransaction() {
      const now = new Date();
      const monthAmount = this.transaction.list.reduce((accumulator, currentValue) => {
        const time = new Date(currentValue.created_at);
    ...
    getMonthPayment() {
      const now = new Date();
      const monthAmount = this.payment.list.reduce((accumulator, currentValue) => {
        const time = new Date(currentValue.paid_at);
        if (time.getMonth() === now.getMonth() && time.getFullYear() === now.getFullYear()) {
          let amount = currentValue.number_package * currentValue.package_price.price;
          return accumulator + amount;
