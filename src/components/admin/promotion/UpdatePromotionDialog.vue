  ...
  >
    <template #header>
      <div class="dialog-header">
        <h2 class="dialog-title">Chỉnh sửa chương trình khuyến mãi</h2>
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="updatePromotion()"> Xác nhận </el-button>
      </span>
    </template>
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    async updatePromotion() {
      ...
      const res = await PromotionService.updatePromotion(this.promotion.id, obj);
      if (res.status === 200) {
        this.$store.state.toast.success('Sửa chương trình khuyến mãi thành công!');
        this.closeDialog();
        this.getPromotions();
    ...
    setupData() {
      this.info.name = this.promotion.name;
      this.info.time[0] = this.promotion.start_at;
      this.info.time[1] = this.promotion.end_at;
      this.promotion.childs.forEach(child => {
        if (child.end_range === 999999999) {
          this.info.childs.push({
            start_range: child.start_range,
            end_range: null,
            percent: child.percent,
          });
        } else {
          this.info.childs.push(child);
        }
      });
    },
    checkInfo() {
