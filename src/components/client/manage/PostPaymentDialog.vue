  ...
  >
    <template #header>
      <div class="dialog-header">
        <h2 v-if="isExtend" class="dialog-title">Gia hạn bài đăng #{{ post.id }}</h2>
        <h2 v-else class="dialog-title">Thanh toán bài đăng #{{ post.id }}</h2>
    ...
    <div class="dialog-body">
      <div class="payment-container">
        <div class="payment-content">
            ...
            <ul class="list-type">
              ...
              >
                <input
                  ...
                  :id="`type-${index}`"
                  :disabled="isExtend && info.postType !== postType.value"
                  v-model="info.postType"
                />
                <label :for="`type-${index}`">{{ postType.name }}</label>
              </div>
            </ul>
          </div>
          <div class="inp-group">
              ...
              >
                ...
                <label :for="`package-price-${packagePrice.id}`"
                  ...
                  <div class="time">
                    ...
                    <span
                      ><Strong>{{ packagePrice.number_of_days }} ngày</Strong> kể từ khi đăng
                      ký</span
                    >
                  </div>
                </label>
          ...
          <div class="inp-group">
            <label class="main-label" for="number-package">Số lượng gói tin</label>
            <el-input-number
              id="number-package"
              :min="1"
              v-model="info.number"
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="purchase()">Thanh toán</el-button>
      </div>
    </template>
  </el-dialog>
</template>
...
export default {
  ...
  computed: {
    ...
    listPackagePrice() {
      return this.packagePrices.filter(packagePrice => packagePrice.type === this.info.postType);
    },
  },
  methods: {
    ...
    checkInfo() {
      const packagePrice = this.packagePrices.find(p => p.id === this.info.packagePrice);
